...
# Module 7 QUERIES (QUESTIONS 1 - 10)
CREATE VIEW SMALL_PROPERTY AS
	(SELECT PROPERTY_ID, OFFICE_NUM, BDRMS, MONTHLY_RENT, OWNER_NUM
		FROM PROPERTY
			WHERE (SQR_FT < 1250)
);
SELECT*
...
CREATE VIEW PROPERTY_OWNERS AS
	(SELECT PROPERTY_ID, OFFICE_NUM, BDRMS, SQR_FT, FLOORS, MONTHLY_RENT, LAST_NAME
		FROM PROPERTY, OWNER
...
CREATE VIEW MONTHLY_RENTS (BDRMS, AVERAGE_RENT, SQR_FT) AS 
	(SELECT BDRMS, AVG(MONTHLY_RENT), SQR_FT
		FROM PROPERTY
			GROUP BY BDRMS
            ORDER BY BDRMS
	);
SELECT*
	FROM MONTHLY_RENTS; 
SELECT SQR_FT, AVERAGE_RENT
	FROM MONTHLY_RENTS
    WHERE (AVERAGE_RENT > 1100); 	
SELECT BDRMS, SQR_FT, AVG(MONTHLY_RENT) AS AVERAGE_RENT
	FROM PROPERTY
		GROUP BY BDRMS
		HAVING (AVERAGE_RENT > 1100)
        ORDER BY BDRMS;
GRANT SELECT ON PROPERTY TO OLIVER; 
GRANT INSERT ON PROPERTY TO CRANDALL, PEREZ; 
GRANT INSERT ON OWNERS TO CRANDALL, PEREZ;  
...
SELECT*
	FROM INFORMATION_SCHEMA.COLUMNS
		WHERE (TABLE_SCHEMA = 'MODULE_3_CASE_STUDY') AND (TABLE_NAME = 'PROPERTY');
SELECT*
	FROM INFORMATION_SCHEMA.COLUMNS
		WHERE (TABLE_SCHEMA = 'MODULE_3_CASE_STUDY') AND (COLUMN_NAME = 'OWNER_NUM'); 
